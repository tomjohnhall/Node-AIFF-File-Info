    ...
    fs.read(fd, buffer, 0, 80, 0, function(err, num) {
      ...
      function read_aif() {
        ...
        if (i == 5) {
          ...
          if (
            read_result.aiff_identifier == 'AIFC' &&
            read_result.comm_identifier == 'FVER'
          ) {
      ...
      function post_process() {
        ...
        if (read_result.form_identifier != 'FORM')
          invalid_reasons.push('Expected "FORM" string at 0')
        if (
          read_result.aiff_identifier != 'AIFF' &&
          read_result.aiff_identifier != 'AIFC'
        )
          invalid_reasons.push('Expected "AIFF" or "AIFC" string')
        if (
          read_result.comm_identifier != 'COMM' &&
          read_result.comm_identifier != 'FVER'
        )
          invalid_reasons.push('Expected "COMM" or "FVER" string')
